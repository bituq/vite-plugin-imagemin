...
const imageminOptPng__default =
  /*#__PURE__*/ _interopDefaultLegacy(imageminOptPng)
...
const imageminJpegTran__default =
  /*#__PURE__*/ _interopDefaultLegacy(imageminJpegTran)
...
function index(options = {}) {
  ...
  return {
    ...
    configResolved(resolvedConfig) {
      ...
      outputPath = path__default$1.resolve(
        path__default$1.resolve(config.root, config.build.outDir),
      )
    ...
    async generateBundle(_, bundler) {
      ...
      Object.keys(bundler).forEach((key) => {
        filterFile(path__default$1.resolve(outputPath, key), filter) &&
          files.push(key)
      })
      debug('files:', files)
      if (!files.length) {
      ...
      if (publicDir) {
        ...
        readAllFiles(publicDir).forEach((file) => {
          filterFile(file, filter) &&
            files.push(path__default$1.relative(publicDir, file))
        })
        if (files.length) {
          const handles = files.map(async (filePath) => {
...
${chalk__default.cyan(
  ...
  recordMap.forEach((value, name) => {
    ...
    const denseRatio =
      ratio > 0
        ? chalk__default.red(`+${fr}%`)
        : ratio <= 0
        ? chalk__default.green(`${fr}%`)
        : ''
    const sizeStr = `${oldSize.toFixed(2)}kb / tiny: ${size.toFixed(2)}kb`
    config.logger.info(
      chalk__default.dim(path__default$1.basename(config.build.outDir)) +
        '/' +
        chalk__default.blueBright(name) +
        ' '.repeat(2 + maxKeyLength - name.length) +
        chalk__default.gray(
          `${denseRatio} ${' '.repeat(valueKeyLength - fr.length)}`,
        ) +
        ' ' +
        chalk__default.dim(sizeStr),
    )
  })
  config.logger.info('\n')
}
