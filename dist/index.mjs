...
function index(options = {}) {
  ...
  return {
    ...
    configResolved(resolvedConfig) {
      ...
      outputPath = path$1.resolve(
        path$1.resolve(config.root, config.build.outDir),
      )
      ...
      if (publicDir) {
        ...
        readAllFiles(publicDir).forEach((file) => {
          filterFile(file, filter) &&
            files.push(path$1.relative(publicDir, file))
        })
        if (files.length) {
          const handles = files.map(async (filePath) => {
...
${chalk.cyan(
  ...
  recordMap.forEach((value, name) => {
    ...
    config.logger.info(
      chalk.dim(path$1.basename(config.build.outDir)) +
        '/' +
